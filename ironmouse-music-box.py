...
if __name__ == '__main__':
    ...
    lines = [
        f"Nothing else could sway my heart ",
        f"From the start ",
        f"You shone brighter than Heaven's star ",
        f"Even far apart ",
        f"Nothing will sway my heart ",
        f"Your music box ",
        f"Rings louder than the dying stars ",
        f"Eternally bound to you",
        f"Forever I'll burn for you ",
        f"So with the last of the strength in my heart",
        f"I'd burn it all ",
        f"For you ",
    ]
    ...
    for i in range(0, 5):
        photo = Image.open(f"source-imgs/ironmouse/1_1.jpg")
        w, h = photo.size
        ...
        for i in range(3):
            draw_stripe_of_text(lines[1], draw, TextMode.LETTERS, w, h)
            draw_stripe_of_text(lines[1], draw, TextMode.LETTERS, w, h)
            draw_stripe_of_text(lines[1], draw, TextMode.WORDS, w, h)
        ...
        imgOut.save(f"pallette-out/ironmouse/ironmouse-{time()}-{i}.png")
