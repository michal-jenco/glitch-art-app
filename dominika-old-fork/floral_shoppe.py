...
def fractally_func_4(r, g, b, h, w, f1, f2, f3,
    ...
    base_wave_size = 250
    new_r = r + .09 * (f1(b)/3 + h -(w * (i + 1))) / (base_wave_size + wave_size_modifier_r) * red_amount
    new_g = g + .09 * (f2(r)/3 + r) / (base_wave_size + wave_size_modifier_g) * green_amount
    new_b = b + .09 * (f3(g)/3 - h +(w / (i + 1))) / (base_wave_size + wave_size_modifier_b) * blue_amount
...
def generate_consecutive_palettes(img: Image,
    ...
    for i in range(2, image_count + 2):
        ...
        img_save_name = f"pallette-out/floral-shoppe/{int(time())}-{i}.png"
